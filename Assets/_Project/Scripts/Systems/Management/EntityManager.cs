...
/// </summary>
{
    public class EntityManager : MonoBehaviour
    {
        ...
        /// All entity pools.
        /// </summary>
        protected Dictionary<EntityType, Pool<EntityBase>> pools { get; } = new();
        /// <summary>
        ...
        {
            Pool<EntityBase> pool;
            if (pools.TryGetValue(entity.GetID(), out pool))
            {
                pool.Release(entity);
                return;
            }
            //we need to add a new pool
            pool = new Pool<EntityBase>();
            pool.Release(entity);
            pools.Add(entity.GetID(), pool);
        }
        /// <summary>
